...
describe("HTTP Control Room client", () => {
  it("maps the persisted core snapshot and derives only real pending approvals", async () => {
    const fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify({
      projects: [],
      tasks: [{ id: "task-1", projectId: "project-1", targetDirectoryId: "directory-1", usesWorktree: false, title: "Review", prompt: "Inspect", priority: "critical", status: "waiting_approval", kind: "review", worktreePath: null, createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-01-01T00:00:00.000Z" }],
      executions: [], logs: []
    }), { status: 200 }));
  ...
  it("sends directory collections and an explicit task target without importing host contracts", async () => {
    ...
    await client.createTask({ projectId: "project-1", targetDirectoryId: "directory-web", title: "Tela", prompt: "Implemente", priority: "normal" });
    ...
    expect(fetch).toHaveBeenNthCalledWith(2, expect.stringContaining("/tasks"), expect.objectContaining({ body: expect.stringContaining("targetDirectoryId") }));
  });
});
